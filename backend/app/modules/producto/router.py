...
async def update(
    ...
    with ProductoUnitOfWork() as uow:
        service = ProductoService(uow)
        producto = service.get_orm_by_id(producto_id)
...
async def delete(
    ...
    with ProductoUnitOfWork() as uow:
        service = ProductoService(uow)
        producto = service.get_orm_by_id(producto_id, include_deleted=True)
...
async def update_imagenes(
    ...
    with ProductoUnitOfWork() as uow:
        ...
        if not producto:
            raise HTTPException(status_code=404, detail="Producto no encontrado")
        resultado = service.update(producto, {"imagenes_url": data.imagenes_url})
...
def get_ingredientes(producto_id: Annotated[int, Path(gt=0)]):
    with ProductoUnitOfWork() as uow:
        stmt = select(Producto).where(Producto.id == producto_id).options(selectinload(Producto.ingredientes))
        orm_producto = uow.session.exec(stmt).first()
        if not orm_producto:
            raise HTTPException(status_code=404, detail="Producto no encontrado")
        return orm_producto.ingredientes
