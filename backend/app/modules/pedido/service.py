    ...
    def cambiar_estado(
        ...
        if estado_nuevo == EstadoPedido.ENTREGADO:
            for detalle in pedido.detalles:
                ...
                # Descontar stock de los ingredientes del producto
                pi_records = self.uow.session.exec(
                ...
                for pi in pi_records:
                    ingrediente = self.uow.session.get(Ingrediente, pi.ingrediente_id)
                    if not ingrediente:
                        continue

                    # cantidad del ingrediente × cantidad de producto vendido
                    cantidad_ing = (pi.cantidad or 1) * detalle.cantidad
                    nuevo_stock_ing = ingrediente.stock_cantidad - int(cantidad_ing)
                    ingrediente.stock_cantidad = max(0, nuevo_stock_ing)
                    self.uow.session.add(ingrediente)
