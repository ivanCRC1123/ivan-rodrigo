...
def get_usuario_rol_service(session: Session = Depends(get_session)) -> UsuarioRolService:
    return UsuarioRolService(session)
...
def obtener_roles_usuario(
    ...
    try:
        roles = service.obtener_roles_usuario(usuario_id)
        return roles
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
